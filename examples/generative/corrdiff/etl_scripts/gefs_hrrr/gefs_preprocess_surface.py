...
for year in range(2020, 2025):
    ...
    for i in range(0,2):
        ...
        target_path = f'/lustre/fsw/coreai_climate_earth2/datasets/gefs/twc_mvp/GEFS_surface_{year}{fstr}.zarr'
        store = zarr.DirectoryStore(target_path)
        tic = time.time()
        
        # Initialize the Zarr store if it doesn't exist
        grbs = grid_read(timestamps[0], 0, res=25)
        ...
        for timestamp in timestamps:
            for lead_time in range(0, 25, 3):
                ...
                try:
                    ...
                    continue
                values = []
                for channel in surface_source_channels:
                ...
                data_array = xr.DataArray(
                    ...
                    coords={"time": [date_str], "channel": surface_channels, "x": x, "y": y},
                ).chunk({"time": 1, "channel": len(surface_channels), "x": 721, "y": 1440})
