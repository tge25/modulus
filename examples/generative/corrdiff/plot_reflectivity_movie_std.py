...
path = "/lustre/fsw/coreai_climate_earth2/corrdiff/inferences/twc_mvp_correcteddiffusion_mse_test_2_0.nc" #"image_outdir_val_paper_plot_0.nc"
...
for i in range(len(time)):
    ...
    var = "cat_rain"
    temp = ds_truth[var].clip(min=0)[i, :, :]
    print((np.sum(temp<=0.05) + np.sum(temp>=0.95))/temp.size)
    temp = ds_prediction[var].clip(min=0).mean(dim="ensemble")[i, :, :]
    print((np.sum(temp<=0.05) + np.sum(temp>=0.95))/temp.size)
    ...
    temp = ds_prediction[var].clip(min=0).mean(dim="ensemble")[i, :, :]
    print((np.sum(temp<=0.05) + np.sum(temp>=0.95))/temp.size)
    ax = plt.subplot(4, ncolumns, 17, projection=ccrs.PlateCarree())
    gl = ax.gridlines(
        crs=ccrs.PlateCarree(),
        color="black",
        alpha=0.0,
    ...
    ax = plt.subplot(4, ncolumns, 18, projection=ccrs.PlateCarree())
    gl = ax.gridlines(
        crs=ccrs.PlateCarree(),
        color="black",
        alpha=0.0,
    ...
    ax = plt.subplot(4, ncolumns, 19, projection=ccrs.PlateCarree())
    gl = ax.gridlines(
        crs=ccrs.PlateCarree(),
        color="black",
        alpha=0.0,
    ...
    ax = plt.subplot(4, ncolumns, 20, projection=ccrs.PlateCarree())
    gl = ax.gridlines(
        crs=ccrs.PlateCarree(),
        color="black",
        alpha=0.0,
