...
infile = '/lustre/fsw/coreai_climate_earth2/corrdiff/inferences/twc_mvp_v3_ens_twc_with_truth_ens0_0.nc'
nc = Dataset(infile, 'r')
...
for ens in range(6,10):

    infile = f'/lustre/fsw/coreai_climate_earth2/corrdiff/inferences/twc_mvp_v3_ens_twc_with_truth_ens{ens}_0.nc'
    nc = Dataset(infile, 'r')

    for imember in range(0,4):
        ...
        for plot in range(4):
            ...
            if plot<2:  
                ...
                m.drawstates(linewidth=0.4,color='Gray')
                m.drawcounties(linewidth=0.1,color='LightGray')
            print(ens, imember, plot, flush=True)
        # ---- set plot title and save 
        plt.subplots_adjust(hspace=0.4)
        plt.suptitle(f"GEFS member {ens} - CorrDiff member {imember} | {cyyyymmddhh}-F{clead_hours:02d}h", fontsize=25, y=0.98)  # Adjust y to move title up
        # Adjust the layout to avoid overlap
        plt.tight_layout(rect=[0, 0, 1, 0.97])  # Adjust rect to ensure suptitle and subplots do not overlap
        plot_title = './output_movie/'+cyyyymmddhh+f'{clead_hours:02d}{ens:02d}{imember:02d}.png'
...
import imageio 
import os
writer = imageio.get_writer(f'precip_ens_0530.mp4', fps = 4)
